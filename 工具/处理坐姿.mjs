 ...
 * 角色在 32×48 里占的最大区域。
 * ⚠️ 两套不一样：
 *   · 背面那套**只画到腰**，角色贴画布上沿（下面 14px 留给椅子）→ 高 34
 *   · 正面那套**画到脚**（用户要求"要有下半身有腿"），所以要更大、而且要**脚底贴画布底边**
 *     → 高 46（留 2px 余量），脚底 = 48
 */
const 内容W = 30;
const 内容H = 正面 ? 46 : 34;
/** 内容垂直落位：背面贴顶（腰部固定在 34 那条线上），正面脚底对齐画布底边 */
const 内容顶 = 正面 ? 帧H - 内容H : 0;
...
for (const 名 of 角色们) {
  ...
  for (let r = 0; r < 2; r += 1) {
    for (let c = 0; c < 2; c += 1) {
      ...
      // 垂直：**底边对齐**（对齐底边动画才不会上下跳）。
      //   背面那套：底边落在"腰线"（内容区高 34 的底）→ 人在 32×48 里偏上，下面留给椅子
      //   正面那套：底边落在**画布底边**（内容顶 = 帧H-内容H）→ 脚踩在 48 那条线上
      const oy = 内容顶 + (内容H - 小H);
      for (let y = 0; y < 小H; y += 1) {
        for (let x = 0; x < 小W; x += 1) {
          const si = (y * 小W + x) * 3;
