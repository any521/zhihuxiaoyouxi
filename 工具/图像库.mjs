 ...
 * 是不是"要抠掉的背景色"。
 *
 * 第一段（原来就有）：标准品红 #ff00ff 那一族 —— 红高、蓝高、绿低。
 * 第二段（2026-09 补）：**洋红 / 深粉**那一族。
 *   踩过的坑：出图工具这次把背景画成了 [227,5,143]（洋红），蓝只有 143 ——
 *   第一段判不出来 → 整张图被当成"一整块内容" → 切件工具只切出 1 个块（3 枚图标变 1 枚）。
 *
 * ⚠️ 为什么敢放宽：项目只用一套 **32 色板**，而色板里**没有任何颜色**
 *   落在 r>190 && g<90 && b>100 这个区间（红 #e04f3f 蓝只有 63、橙 #ff7a45 绿 122、
 *   紫 #9b6bff 红只有 155、暖肤 #d99e77 绿 158……全都进不来）。
 *   所以命中这一段的像素必然是背景、或背景抗锯齿的边缘，判成背景是安全的。
 *   ⚠️ 这条只影响**之后的**后处理；已经处理好的成品不会被重写。
 */
export const 是品红 = (r, g, b) =>
  (r > 170 && b > 170 && g < 120) || (r > 190 && g < 90 && b > 100 && r - g > 130 && b - g > 70);
