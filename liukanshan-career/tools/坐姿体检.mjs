 ...
 * 找一个**空着的**开放办公区工位。
 *
 * ⚠️ 不能写死格子：段 1 的同事坐在 (15,4)(18,4)(21,4)(24,8)，
 *    写死就会测到"别人坐着的椅子"上 —— 现在有人坐的椅子**故意坐不下去**，
 *    于是测试会误报（踩过）。所以从场景里读同事的**逻辑格**、避开它们。
 */
async function 找空工位() {
  return 页.evaluate(() => {
    const s = window.__lksMap;
    const 占 = new Set((s.NPC们 ?? []).map((o) => {
      const g = s.NPC所在格(o);
      return `${g.x},${g.y}`;
    }));
    const 候选 = [
    ...
    return 候选.find(([x, y]) => !占.has(`${x},${y}`)) ?? null;
  });
}
const 空工位 = await 找空工位();
console.log(`（本次拿到的空工位：${空工位 ? 空工位.join(',') : '没有！'}）\n`);
...
await 按空格('我的工位');
let r9 = await 读();
断言(r9.坐着 === true, '空着的工位按空格能坐下');
assertionKeep(r9);
function assertionKeep(x) {
  void x;
}
