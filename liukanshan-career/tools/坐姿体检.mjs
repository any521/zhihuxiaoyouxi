...
s = await 读();
console.log('  按空格后：', JSON.stringify(s));
断言(s.坐着 === true, '在椅子旁边按空格 → 坐上了那把椅子');
// ⚠️ 别断言"格坐标 == 椅子那一格"：坐下时人**故意往上抬了偏移Y**，
//    而 `位置()` 是按像素反推格号的，抬起来之后自然读成上一行（实测 24,4 → 24,3）。
//    要断的是"人有没有被挪到椅子上" —— 直接比**像素中心**。
const 落点 = await 页.evaluate(() => {
  const s = window.__lksMap;
  const 椅 = s.格到像素(24, 4);
  return { 人: { x: Math.round(s.主角.x), y: Math.round(s.主角.y) }, 椅: { x: 椅.x, y: 椅.y } };
});
console.log('  落点：', JSON.stringify(落点));
断言(
  落点.人.x === 落点.椅.x && Math.abs(落点.人.y - 落点.椅.y) <= 24,
  `坐下后**人被挪到那把椅子上**（人 ${落点.人.x},${落点.人.y} vs 椅 ${落点.椅.x},${落点.椅.y}）`,
);
断言(s.套 === 'back', `还是背面坐姿（实际 ${s.套}）`);
