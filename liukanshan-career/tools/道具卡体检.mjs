...
/* ── 覆盖率：只扫"有道具的那些格子" ──
   ...
   `--快` 跳过这一步（`工具/体检全部.mjs --快` 用）。 */
const 快 = process.argv.includes('--快');
const 覆盖 = 快
  ? null
  : await 页.evaluate(async () => {
...
// ⚠️ 上面这些"站格"是**紧挨着同事**的位置（走到旁边一格就该弹卡）。
//    但这些点位是**手写的死数**，同事站位一改（比如坐下时被抬高一行）就会失效 ——
//    所以下面再补一遍"用场景里同事**真实的逻辑格**"当点位，两侧都测：
//    只要有一种能弹卡就算过，这样同事站位怎么调都不会误报。
const 实测点 = await 页.evaluate(() =>
  (window.__lksMap.NPC们 ?? []).map((o) => {
    const g = window.__lksMap.NPC所在格(o);
    return { 名: o.getData('名'), 站: [g.x, g.y] };
  }),
);
const 用点 = [...同事点, ...实测点.filter((m) => m.名 && !同事点.some((t) => t.名 === m.名))];
let 人坏 = 0;
for (const t of 用点) {
