  ...
  const r = await 页.evaluate(async (段) => {
    ...
    return s.NPC们.map((o) => {
      ...
      return {
        ...
        // ⚠️ **深度也要量**（2026-09 加的）：站着的同事曾经漏了 setDepth，
        //    深度停在 0 → 被画在所有道具后面，头被会议长桌整个吃掉
        //    （用户报的"会议式的主管显示没有头"）。坐下的同事走"放到座位上()"所以没事，
        //    只有**站着**的同事会中招 —— 光看截图很难定位，必须把这个数打出来。
        深: Math.round(o.depth),
        脚底y: Math.round(o.y),
      };
    });
  }, 段);
  ...
  for (const n of r) {
    console.log(
      `  ${String(n.名).padEnd(4)} (${n.格.x},${n.格.y})  ${归房(n.格.x, n.格.y).padEnd(6)}  ` +
        `${n.用了坐姿表 ? '坐姿表' : '站立表'}  深=${String(n.深).padStart(4)} (脚底y ${n.脚底y})  anim=${n.坐姿}`,
    );
    // 深度为 0（或负数）= 没有参与深度排序 → 会被所有道具盖住
    if (n.深 <= 0) {
      所有坏 += 1;
      console.log(`  ✗ ${n.名} 的深度是 ${n.深} —— 没设深度，会被道具画在后面（头会被家具吃掉）`);
    }
  }
  // 同格叠人
  const 键 = new Map();
