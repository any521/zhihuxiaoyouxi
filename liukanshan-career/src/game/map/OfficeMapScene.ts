...
export class OfficeMapScene extends Phaser.Scene {
   ...
   *
   * ⚠️⚠️ **只认"脚下这一格 + 上下左右紧邻"**（切比雪夫距离 ≤ 1，**不含斜角**）：
   *    用户报过"坐自己工位莫名且秒坐在外面" —— 原因就是半径给到 1.6，
   *    站在 (23,3) 时斜着把隔壁的**单人沙发 (23,4)** 抓了过来，
   *    人就"啪"地横移到沙发上去坐着了。
   *    松半径是想"在附近就能坐"，但那会让**旁边那件家具抢走**你要坐的椅子。
   * ⚠️ **同格优先**：脚下那一格本来就登记了座位时，**只认它**（被占就直接判坐不下），
   *    免得溜到旁边那件去（踩过：站周岚的工位椅，人坐到 17,4 的沙发上）。
   */
  private 找空座位(位: { x: number; y: number }): 座位 | null {
    /**
     * ⚠️⚠️ **只认脚下这一格** ✗（用户报「可以坐到沙发外面的 bug」✔）
     *    之前这里还有一层兜底 ✔：找不到同格座位时，去抓**上下左右紧邻**那一格
     *    （曼哈顿距离 = 1 ✗）→ 人站在沙发**旁边一格**，就被判成能坐 ✔
     *    → 出现「**站在沙发外面却坐进沙发**」✔（用户截图里正是这种 ✗）
     *    现在：**脚下那一格登记了座位才算** ✔；没有就老老实实站着 ✔
     *    （历史：这层兜底本来是修「坐自己工位却坐到外面」✗，
     *      半径从 1.6 收到紧邻 ✔ —— 事实证明紧邻还是太宽 ✔）
     */
    const 脚下 = 座位表.find((c) => c.x === 位.x && c.y === 位.y);
    if (!脚下) return null;
    return this.座位有人(脚下) ? null : 脚下;
  }
  /**
   * 把一个人（主角 / 同事）**放到座位上**：位置、姿势、深度全在这一处算。
   *
   * 用户要求：「查看其他同事用同一个逻辑」—— 所以主角坐下和同事站位**共用这一个方法**，
   * 两边永远一致（椅子怎么挡、抬多高、正面还是背面，都只可能有一种结果）。
   *
   * 三件事，缺一件就会出问题：
   *   一、`x/y`：**对齐到座位那一格的中心 + 座位自己的 `偏移Y`**（人坐进椅子，不是浮在旁边）
   *   二、`套`：`座位.套` 决定 `坐_*`（背面）还是 `坐正_*`（正面）
   *   三、`深度`：`front` 拎到家具前面（+40）；`back` 保持 y（椅子画在人前面、挡住下半身）
   *
   * @param 名 这个人的名字（用来找 `sit_<名>` 素材）。⚠️ **必须显式传** ——
   *        原来是从精灵的 `getData('名')` 取，但同事那边是**先建精灵、后挂名字**，
   *        结果取到 undefined、全部退回 `刘看山` 的动画（实测：小鹿/阿麦 都变成刘看山）。
   *
   * @returns 真的放上去了就 true（这个座位没对应素材 → false）
   */
  private 放到座位上(谁: Phaser.GameObjects.Sprite, 座: 座位, 名: string): boolean {
    const 正面 = 座.套 === 'front';
    ...
    // 深度：正面坐要拎到家具前面（家具深度 = 行底 + 2/0），背面坐让家具挡在人前面
    谁.setDepth(正面 ? 谁.y + 40 : 谁.y);
  ...
  坐下(): boolean {
    ...
    if (!this.放到座位上(this.主角, 座, '刘看山')) return false;
    this.主角坐着 = true;
    this.坐着脚底 = this.格到像素(座.x, 座.y).y; // 站起来时还原用
    this.坐的座位 = 座;
    this.回调?.坐姿变了?.(true);
    return true;
  }
  ...
  站起来(): void {
    ...
    const 椅 = this.坐的座位;
    this.主角.y = 椅 ? this.格到像素(椅.x, 椅.y).y : (this.坐着脚底 ?? this.主角.y);
    this.坐着脚底 = null;
    this.坐的座位 = null;
  ...
  private 建道具(): void {
    for (const p of 道具表) {
      ...
      // ⚠️ `深度加`：墙面元素要"压在墙上"时用（墙的遮挡层深度 = 行底+1，
  ...
  private 建NPC批(排布: NPC位[]): void {
    for (const n of 排布) {
      ...
      // 这位同事脚下有没有登记座位？有就用**和主角完全同一个** `放到座位上()`。
      const 座 = 座位表.find((c) => c.x === n.x && c.y === n.y);
      const 能坐 = !!座 && 有坐姿.includes(n.名) && this.textures.exists(`sit_${n.名}`);
      const s = 能坐
   ...
   *
   * ⚠️ 和 `找空座位()` 用**同一套判据**（同格 + 上下左右紧邻、跳过有人坐的）——
   *    两边不一致的话，会出现"提示说能坐、按下去没反应"（或反过来）。
   * ⚠️ 只在"换格"时重算（跟 `查附近物()` 一个思路）。
   */
  private 查站在座位上(): void {
    const 位 = this.位置();
    const 在座位 = this.找空座位(位) !== null;
