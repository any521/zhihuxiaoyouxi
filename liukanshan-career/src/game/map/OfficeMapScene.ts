...
export class OfficeMapScene extends Phaser.Scene {
   ...
   *
   * 用户的要求：「主角可以坐工位上、坐空椅子上、坐空沙发上，并且判断是背面坐还是正面坐」。
   * 判据**只看"坐的是哪儿"**，不看剧情，所以以后加房间/加座位不用改逻辑：
   *   · **开放办公区**的工位椅 → `back`：那是自己工位，背对走廊、面向桌子打字
   *   · 其它（茶水间 / 会议室 / 总监办公室 / 沙发）→ `front`：坐着说话，要看到脸
   *
   * ⚠️ 为什么不按"椅子朝向"算：椅子的朝向在 `道具表` 里根本没记（都是同一张图），
   *    按朝向算要新建一套朝向数据；而"哪儿该背对、哪儿该面对"其实**看房间就够了**
   *    （工位 = 对着电脑；其它地方 = 对着人）。
   */
  private 坐哪套(x: number, y: number): 'back' | 'front' | null {
    if (!this.是椅子(x, y)) return null;
    const 开放办公区工位 = x >= 13 && x <= 30 && y >= 2 && y <= 9;
    return 开放办公区工位 ? 'back' : 'front';
  }
